...
- **Offline compression wrote transcripts into IRONMEM.md** — `CompressionMode::Local` set a memory's `summary` to the session transcript truncated at 48 KB. `ironmem inject` writes summaries into IRONMEM.md, and `CLAUDE.md` imports that file, so every offline session added up to 48 KB of raw tool output to the top of every later session in the project. One project's file reached 166 KB, roughly 45k tokens loaded before any work began. The local summary is now a deterministic digest (observation count, tool histogram, extracted facts, capped at 2 KB); the verbatim transcript is unaffected and stays retrievable through CCR and `retrieve_original`, where it always was.
- **IRONMEM.md had no size ceiling** — the generated file now caps each memory's summary at 4 KB and the whole file at 24 KB, noting how many older memories were omitted and that `search_memories` still reaches them. This bounds the file regardless of what a provider returns, and shrinks existing bloated memories on the next write without touching stored data.
