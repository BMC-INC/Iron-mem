...
No production database migration, installed service restart or paid model evaluation is part of this batch. Telemetry and opt-in working sets are implemented; interning remains measurement-gated. See [access measurements](ACCESS-WORKING-SETS.md) and [temporal assertion semantics](../../architecture/temporal-assertions.md) for the subsequent batches. Assertion history is included in full/delta checkpoints and cannot be rewound by restoring an older checkpoint.
